#!/usr/bin/env python3
# send-southbury-docs.py
# Run the Southbury CT agenda downloader and email all new files.
#
# CONFIGURATION:
...
SCRIPT     = os.path.join(REPO_DIR, "scripts", "download-southbury-agendas.py")
OUTPUT_DIR = os.path.join(REPO_DIR, "beat-archive", "southbury-agendas")
CITY_NAME  = "SOUTHBURY"
...
def run_downloader():
    print("Running download-southbury-agendas.py ...")
...
def send_email(files, downloader_output, biz_table=""):
    subject = (
        f"Southbury CT meeting docs — {datetime.date.today().strftime('%B %-d, %Y')} "
    ...
    body = (
        f"Southbury CT agenda/minutes download — {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}\n\n"
