#!/usr/bin/env python3
# send-farmington-docs.py
# Run the Farmington CT agenda downloader and email all new files.
#
# CONFIGURATION:
...
SCRIPT     = os.path.join(REPO_DIR, "scripts", "download-farmington-agendas.py")
OUTPUT_DIR = os.path.join(REPO_DIR, "beat-archive", "farmington-agendas")
...
def run_downloader():
    print("Running download-farmington-agendas.py ...")
...
def send_email(files, downloader_output, biz_table=""):
    subject = (
        f"Farmington CT meeting docs — {datetime.date.today().strftime('%B %-d, %Y')} "
    ...
    body = (
        f"Farmington CT agenda/minutes download — {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}\n\n"
