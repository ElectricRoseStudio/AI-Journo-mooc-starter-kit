#!/usr/bin/env python3
# send-canton-docs.py
# Run the Canton CT agenda downloader and email all new files.
#
# CONFIGURATION:
...
SCRIPT     = os.path.join(REPO_DIR, "scripts", "download-canton-agendas.py")
OUTPUT_DIR = os.path.join(REPO_DIR, "beat-archive", "canton-agendas")
...
def run_downloader():
    print("Running download-canton-agendas.py ...")
...
def send_email(files, downloader_output):
    ...
    body = (
        f"Canton CT agenda/minutes download — {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}\n\n"
