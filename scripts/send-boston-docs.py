#!/usr/bin/env python3
# send-boston-docs.py
# Run the Boston MA agenda downloader and email all new files.
#
# CONFIGURATION:
...
SCRIPT     = os.path.join(REPO_DIR, "scripts", "download-boston-agendas.py")
OUTPUT_DIR = os.path.join(REPO_DIR, "beat-archive/boston-agendas")
TOWN_NAME  = "Boston, MA"
...
def run_downloader():
    print("Running download-boston-agendas.py ...")
...
def send_email(files, downloader_output):
    ...
    if skipped:
        skipped_note = (
            ...
            "and were not attached (Boston's City Council/committee video via "
            "Granicus routinely runs large):\n"
            + "\n".join(
