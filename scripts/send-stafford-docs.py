#!/usr/bin/env python3
# send-stafford-docs.py
# Run the Stafford CT agenda downloader and email all new files.
#
# CONFIGURATION:
...
SCRIPT     = os.path.join(REPO_DIR, "scripts", "download-stafford-agendas.py")
OUTPUT_DIR = os.path.join(REPO_DIR, "beat-archive", "stafford-agendas")
CITY_NAME  = "STAFFORD"
...
def run_downloader():
    print("Running download-stafford-agendas.py ...")
...
def send_email(files, downloader_output, biz_table=""):
    ...
    subject = (
        f"Stafford CT meeting docs — {datetime.date.today().strftime('%B %-d, %Y')} "
    ...
    body = (
        f"Stafford CT agenda/minutes download — {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}\n\n"
