#!/usr/bin/env python3
# send-shrewsbury-docs.py
# Run the Shrewsbury MA agenda downloader and email all new files.
#
# CONFIGURATION:
...
SCRIPT     = os.path.join(REPO_DIR, "scripts", "download-shrewsbury-agendas.py")
OUTPUT_DIR = os.path.join(REPO_DIR, "beat-archive/shrewsbury-agendas")
TOWN_NAME  = "Shrewsbury, MA"
...
# .url Internet Shortcut files (non-YouTube video links the downloader
# can't fetch directly) are small and always safe to attach — skip the
# size-budget dance for them so the editor always gets the link.
LINK_EXTENSIONS = {".url"}
...
def run_downloader():
    print("Running download-shrewsbury-agendas.py ...")
