#!/usr/bin/env python3
# download-danvers-agendas.py
# Download municipal meeting agendas, minutes, and video recordings from
# the Danvers MA Agenda Center for meetings within the past N days (and
# up to 7 days ahead).
#
# USAGE:
#   python3 scripts/download-danvers-agendas.py [options]
#
# REQUIREMENTS:
...
#     1. Fetches the Danvers MA Agenda Center search endpoint with a date
...
#     3. Downloads PDFs to beat-archive/danvers-agendas/YYYY-MM/
#
#   Video (--include-video or --video-only):
#     4. Extracts each meeting's single video link, if any, from the row's
#        <td class="media"> cell (same markup as Malden/Shrewsbury MA's
#        Agenda Center) and downloads it via yt-dlp
#
# SITE STRUCTURE (CivicPlus CivicEngage "Agenda Center", same platform as
# Waterford CT and every other MA town in this repo):
#   Hub:     https://www.danversma.gov/AgendaCenter
#   Search:  https://www.danversma.gov/AgendaCenter/Search/?term=&CIDs=all
#              &startDate=MM/DD/YYYY&endDate=MM/DD/YYYY&dateRange=Custom&dateSelector=0
#   Agenda:  https://www.danversma.gov/AgendaCenter/ViewFile/Agenda/_MMDDYYYY-ID
#   Minutes: https://www.danversma.gov/AgendaCenter/ViewFile/Minutes/_MMDDYYYY-ID
#   Video:   one link per meeting (if any) in <td class="media">. Checked
#            a 6-month window directly: mostly YouTube (youtube.com/watch
#            or youtu.be), but some rows link meet.google.com instead —
#            a live-meeting JOIN link, not a recording, with no archival
#            value once the meeting has ended. Only youtube.com/youtu.be
#            links are downloaded; meet.google.com and anything else is
#            skipped outright (not even saved as a .url shortcut, since
#            unlike Malden's SharePoint/Zoom links, which point to real
#            recordings behind a login, a Meet join link isn't a
#            recording at all).
#
# COVERAGE: School Committee (Board of Education) and Recreation
# Committee (Parks & Recreation) are both native Agenda Center categories
...
# --- Configuration ---
BASE_URL = "https://www.danversma.gov"
SEARCH_URL = f"{BASE_URL}/AgendaCenter/Search/"
OUTPUT_DIR = "beat-archive/danvers-agendas"
DAYS_BACK = 4
DAYS_AHEAD = 7
DELAY_SECONDS = 0.8

UA = "Danvers-MA-Agendas-Downloader/1.0 (journalism research)"
...
# Only youtube.com/youtu.be are real recordings — meet.google.com links
# seen on this site are live-meeting join links, not archived video.
_VIDEO_HOST_RE = re.compile(r"https?://(?:www\.)?(youtube\.com|youtu\.be)/", re.IGNORECASE)
...
class AgendaParser(html.parser.HTMLParser):
    """
    Single-pass parser for the Danvers CivicPlus Agenda Center search
    results. Tracks h2 (board name) and h3 (meeting date), collecting
    ViewFile/Agenda, ViewFile/Minutes, and YouTube video links between h3
    boundaries.
    """
    ...
    def handle_starttag(self, tag, attrs):
        ...
        if tag == "h2":
            ...
            # MA and Beverly MA) — strip any query string.
            href = href.split("?", 1)[0]
            lower = href.lower()
            ...
            elif self._video_url is None and _VIDEO_HOST_RE.search(attrs_d.get("href", "")):
                self._video_url = attrs_d.get("href", "")
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Download Danvers MA municipal agendas, minutes, and video recordings "
    ...
    parser.add_argument("--include-video", action="store_true",
                        help="Also download YouTube video recordings via yt-dlp")
...
#    python3 scripts/download-danvers-agendas.py --dry-run
#
# 2. Narrow to one board:
#    python3 scripts/download-danvers-agendas.py --board "School Committee"
#
# 3. Download documents AND video recordings:
#    python3 scripts/download-danvers-agendas.py --include-video
#
# 4. Download only video recordings:
#    python3 scripts/download-danvers-agendas.py --video-only
#
# 5. Change the lookback window:
#    python3 scripts/download-danvers-agendas.py --days 14
#
# 6. Run on a schedule (cron — evening):
#    0 19 * * 1-5 cd /path/to/repo && python3 scripts/download-danvers-agendas.py
