#!/usr/bin/env python3
# send-shelton-docs.py
# Run the Shelton CT agenda downloader and email all new files.
#
# CONFIGURATION:
...
SCRIPT     = os.path.join(REPO_DIR, "scripts", "download-shelton-agendas.py")
OUTPUT_DIR = os.path.join(REPO_DIR, "beat-archive", "shelton-agendas")
...
def run_downloader():
    print("Running download-shelton-agendas.py ...")
...
def send_email(files, downloader_output):
    ...
    subject = (
        f"Shelton CT meeting docs — {datetime.date.today().strftime('%B %-d, %Y')} "
    ...
    body = (
        f"Shelton CT agenda/minutes download — {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}\n\n"
