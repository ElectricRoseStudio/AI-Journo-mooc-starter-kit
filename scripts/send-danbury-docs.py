#!/usr/bin/env python3
# send-danbury-docs.py
# Run the Danbury CT agenda downloader and email all new files.
#
# CONFIGURATION:
...
SCRIPT     = os.path.join(REPO_DIR, "scripts", "download-danbury-agendas.py")
OUTPUT_DIR = os.path.join(REPO_DIR, "beat-archive", "danbury-agendas")

ATTACH_EXTENSIONS = {".pdf", ".url"}
...
def run_downloader():
    print("Running download-danbury-agendas.py ...")
...
def send_email(files, downloader_output):
    subject = (
        f"Danbury CT meeting docs — {datetime.date.today().strftime('%B %-d, %Y')} "
    ...
    body = (
        f"Danbury CT agenda/minutes download — {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}\n\n"
