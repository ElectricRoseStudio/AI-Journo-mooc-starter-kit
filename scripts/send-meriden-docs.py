#!/usr/bin/env python3
# send-meriden-docs.py
# Run the Meriden CT agenda downloader and email all new files.
#
# CONFIGURATION:
...
SCRIPT     = os.path.join(REPO_DIR, "scripts", "download-meriden-agendas.py")
OUTPUT_DIR = os.path.join(REPO_DIR, "beat-archive", "meriden-agendas")
...
def run_downloader():
    print("Running download-meriden-agendas.py ...")
...
def send_email(files, downloader_output):
    ...
    subject = (
        f"Meriden CT meeting docs — {datetime.date.today().strftime('%B %-d, %Y')} "
    ...
    body = (
        f"Meriden CT agenda/minutes download — {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}\n\n"
