#!/usr/bin/env python3
# send-wallingford-docs.py
# Run the Wallingford CT agenda downloader and email all new files.
#
# CONFIGURATION:
...
SCRIPT     = os.path.join(REPO_DIR, "scripts", "download-wallingford-agendas.py")
OUTPUT_DIR = os.path.join(REPO_DIR, "beat-archive", "wallingford-agendas")
...
def run_downloader():
    print("Running download-wallingford-agendas.py ...")
...
def send_email(files, downloader_output):
    ...
    subject = (
        f"Wallingford CT meeting docs — {datetime.date.today().strftime('%B %-d, %Y')} "
    ...
    body = (
        f"Wallingford CT agenda/minutes download — {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}\n\n"
