#!/usr/bin/env python3
# send-stratford-docs.py
# Run the Stratford CT agenda downloader and email all new files.
#
# CONFIGURATION:
...
SCRIPT     = os.path.join(REPO_DIR, "scripts", "download-stratford-agendas.py")
OUTPUT_DIR = os.path.join(REPO_DIR, "beat-archive", "stratford-agendas")
CITY_NAME  = "STRATFORD"
...
def run_downloader():
    print("Running download-stratford-agendas.py ...")
...
def send_email(files, downloader_output, biz_table=""):
    ...
    subject = (
        f"Stratford CT meeting docs — {datetime.date.today().strftime('%B %-d, %Y')} "
    ...
    body = (
        f"Stratford CT agenda/minutes download — {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}\n\n"
