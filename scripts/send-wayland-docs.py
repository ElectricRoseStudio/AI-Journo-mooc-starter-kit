#!/usr/bin/env python3
# send-wayland-docs.py
# Run the Wayland MA agenda downloader and email all new files.
#
# NOTE: Wayland has no reliable video source right now — see
# download-wayland-agendas.py's header for the full investigation. This
# script is documents-only; the email body notes the gap explicitly.
#
# CONFIGURATION:
...
TO_ADDRESS   = "jessie.castellano@patch.com"

REPO_DIR   = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
LOG_PATH   = os.path.join(REPO_DIR, "beat-archive", "send-log.txt")
SCRIPT     = os.path.join(REPO_DIR, "scripts", "download-wayland-agendas.py")
OUTPUT_DIR = os.path.join(REPO_DIR, "beat-archive/wayland-agendas")
TOWN_NAME  = "Wayland, MA"
...
NO_VIDEO_NOTE = (
    "Note: no video source is available for Wayland right now. WayCAM's "
    "YouTube channel carries no government-meeting content, its on-demand "
    "archive link is dead (404), and its own site is mid-transition to a "
    "paid membership model — nothing free/reliable to automate against. "
    "Documents only until that changes.\n"
)
...
def run_downloader():
    print("Running download-wayland-agendas.py ...")
...
def send_email(files, downloader_output):
    ...
    body = (
        f"{TOWN_NAME} agenda/minutes download — "
