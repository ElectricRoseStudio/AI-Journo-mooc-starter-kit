#!/usr/bin/env python3
# send-windsor-locks-docs.py
# Run the Windsor Locks CT agenda downloader and email all new files.
#
# CONFIGURATION:
...
SCRIPT     = os.path.join(REPO_DIR, "scripts", "download-windsor-locks-agendas.py")
OUTPUT_DIR = os.path.join(REPO_DIR, "beat-archive", "windsor-locks-agendas")

ATTACH_EXTENSIONS = {".pdf", ".mp4", ".m4a", ".mp3", ".wav"}
...
def run_downloader():
    print("Running download-windsor-locks-agendas.py ...")
...
def send_email(files, downloader_output):
    ...
    subject = (
        f"Windsor Locks CT meeting docs — {datetime.date.today().strftime('%B %-d, %Y')} "
    ...
    body = (
        f"Windsor Locks CT agenda/minutes download — {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}\n\n"
