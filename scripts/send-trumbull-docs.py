#!/usr/bin/env python3
# send-trumbull-docs.py
# Run the Trumbull CT agenda downloader and email all new files.
#
# CONFIGURATION:
...
SCRIPT     = os.path.join(REPO_DIR, "scripts", "download-trumbull-agendas.py")
OUTPUT_DIR = os.path.join(REPO_DIR, "beat-archive", "trumbull-agendas")
...
def run_downloader():
    print("Running download-trumbull-agendas.py ...")
...
def send_email(files, downloader_output):
    ...
    subject = (
        f"Trumbull CT meeting docs — {datetime.date.today().strftime('%B %-d, %Y')} "
    ...
    body = (
        f"Trumbull CT agenda/minutes download — {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}\n\n"
