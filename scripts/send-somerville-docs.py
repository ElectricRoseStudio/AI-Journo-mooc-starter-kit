#!/usr/bin/env python3
# send-somerville-docs.py
# Run the Somerville MA agenda downloader and email all new files.
#
# CONFIGURATION:
...
SCRIPT     = os.path.join(REPO_DIR, "scripts", "download-somerville-agendas.py")
OUTPUT_DIR = os.path.join(REPO_DIR, "beat-archive/somerville-agendas")
TOWN_NAME  = "Somerville, MA"
...
def run_downloader():
    print("Running download-somerville-agendas.py ...")
...
def send_email(files, downloader_output):
    ...
    if skipped:
        skipped_note = (
            ...
            "and were not attached (Somerville's video files run large — City "
            "Council/committee recordings via Granicus and YouTube video from "
            "other boards routinely exceed the size budget):\n"
            + "\n".join(
