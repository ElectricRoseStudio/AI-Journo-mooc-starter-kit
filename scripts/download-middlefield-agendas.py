#!/usr/bin/env python3
# download-middlefield-agendas.py
# Download municipal meeting agendas, minutes, and (best-effort) video
# recordings from the Middlefield, CT Agenda Center for meetings within
# the past N days (and up to M days ahead).
#
# USAGE:
#   python3 scripts/download-middlefield-agendas.py [options]
#
# REQUIREMENTS:
...
#     1. Fetches the Middlefield CT Agenda Center search endpoint with a
...
#     3. Downloads PDFs to beat-archive/middlefield-agendas/YYYY-MM/
#
...
# Clinton/Haddam/Salem/Swampscott/Woodbridge elsewhere in this repo):
#   Hub:     https://www.middlefieldct.org/AgendaCenter
#   Search:  https://www.middlefieldct.org/AgendaCenter/Search/?term=&CIDs=all
#              &startDate=MM/DD/YYYY&endDate=MM/DD/YYYY&dateRange=Custom&dateSelector=0
#   Agenda:  https://www.middlefieldct.org/AgendaCenter/ViewFile/Agenda/_MMDDYYYY-ID
#   Minutes: https://www.middlefieldct.org/AgendaCenter/ViewFile/Minutes/_MMDDYYYY-ID
#
# VIDEO NOTE — No reliable per-meeting video source was found for
#   Middlefield. Checked directly before writing this script:
#     - No YouTube/Vimeo/Facebook-Live link anywhere on middlefieldct.org
#       (the one "/youtube" shortcut on the site redirects to a staff
#       login page, not a public channel).
#     - The regional Cablecast (Tightrope Media Systems) instance at
#       reflect-vsctv.cablecast.tv — the same platform already used for
#       Durham, Swampscott, North Andover, and Peabody elsewhere in this
#       repo — has no "Middlefield" project and returns only two
#       unrelated community-event videos (Homecoming Celebration, Old
#       Home Days) when searched, nothing meeting-related.
#   Durham (the town Middlefield shares a regional school district and an
#   Interlocal Agreement Advisory Board with) has the exact same gap, so
#   this looks like a genuine absence of recorded/archived meeting video
#   rather than something this script is failing to find. Video search is
#   included anyway for completeness and left OFF by default
#   (--include-video/--video-only to try it) — expect it to find nothing
#   until/unless Middlefield starts recording meetings somewhere
#   discoverable.
...
# --- Configuration ---
BASE_URL = "https://www.middlefieldct.org"
SEARCH_URL = f"{BASE_URL}/AgendaCenter/Search/"
CABLECAST_BASE = "https://reflect-vsctv.cablecast.tv/cablecastapi/v1"
OUTPUT_DIR = "beat-archive/middlefield-agendas"
DAYS_BACK = 4
DAYS_AHEAD = 7
DELAY_SECONDS = 0.8

UA = "Middlefield-CT-Agendas-Downloader/1.0 (journalism research)"
...
def collect_videos(cutoff, future_limit, board_filter, known_boards):
    ...
    for board in sorted(boards):
        for show in fetch_cablecast_shows(search=f"Middlefield {board}"):
            if show["id"] in seen_show_ids:
                continue
            title = show.get("title") or ""
            if "middlefield" not in title.lower() or board.lower() not in title.lower():
                continue
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Download Middlefield CT municipal agendas, minutes, and (best-effort) "
...
#    python3 scripts/download-middlefield-agendas.py --dry-run
#
# 2. Just Board of Selectmen:
#    python3 scripts/download-middlefield-agendas.py --board "Board of Selectmen"
#
# 3. Change the lookback window:
#    python3 scripts/download-middlefield-agendas.py --days 14
#
# 4. Also try Cablecast video search (see VIDEO NOTE — expect little/nothing):
#    python3 scripts/download-middlefield-agendas.py --include-video
#
# 5. Run on a schedule (cron — evening):
#    0 20 * * 1-5 cd /path/to/repo && python3 scripts/download-middlefield-agendas.py
