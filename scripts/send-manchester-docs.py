#!/usr/bin/env python3
# send-manchester-docs.py
# Run the Manchester CT agenda downloader and email all new files.
#
# CONFIGURATION:
...
SCRIPT     = os.path.join(REPO_DIR, "scripts", "download-manchester-agendas.py")
OUTPUT_DIR = os.path.join(REPO_DIR, "beat-archive", "manchester-agendas")
...
def run_downloader():
    print("Running download-manchester-agendas.py ...")
...
def send_email(files, downloader_output):
    ...
    subject = (
        f"Manchester CT meeting docs — {datetime.date.today().strftime('%B %-d, %Y')} "
    ...
    body = (
        f"Manchester CT agenda/minutes download — {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}\n\n"
