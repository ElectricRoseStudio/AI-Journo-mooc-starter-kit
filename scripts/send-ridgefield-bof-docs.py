#!/usr/bin/env python3
# send-ridgefield-bof-docs.py
# Run the Ridgefield CT Board of Finance agenda downloader and email all new files.
#
# CONFIGURATION:
...
SCRIPT     = os.path.join(REPO_DIR, "scripts", "download-ridgefield-bof-agendas.py")
OUTPUT_DIR = os.path.join(REPO_DIR, "beat-archive", "ridgefield-bof-agendas")
TOWN_NAME  = "Ridgefield Board of Finance, CT"
...
def run_downloader():
    print("Running download-ridgefield-bof-agendas.py ...")
...
def collect_recent_files(hours=24):
    """Return list of files under OUTPUT_DIR modified in the past N hours."""
...
def send_email(files, downloader_output):
    ...
    if skipped:
        skipped_note = (
            f"\n{len(skipped)} file(s) exceeded the {MAX_ATTACH_BYTES // (1024*1024)} MB "
