#!/usr/bin/env python3
# send-stonington-docs.py
# Run the Stonington CT agenda downloader and email all new files.
#
# CONFIGURATION:
...
SCRIPT     = os.path.join(REPO_DIR, "scripts", "download-stonington-agendas.py")
OUTPUT_DIR = os.path.join(REPO_DIR, "beat-archive", "stonington-agendas")
...
def run_downloader():
    print("Running download-stonington-agendas.py ...")
...
def send_email(files, downloader_output, biz_table=""):
    ...
    subject = (
        f"Stonington CT meeting docs — {datetime.date.today().strftime('%B %-d, %Y')} "
    ...
    body = (
        f"Stonington CT agenda/minutes download — {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}\n\n"
