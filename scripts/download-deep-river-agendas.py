#!/usr/bin/env python3
# download-deep-river-agendas.py
# Download municipal meeting agendas, minutes, and video recordings from
# the Deep River, CT Agenda Center for meetings within the past N days
# (and up to M days ahead).
#
# USAGE:
#   python3 scripts/download-deep-river-agendas.py [options]
#
# REQUIREMENTS:
...
#     1. Fetches the Deep River CT Agenda Center search endpoint with a
...
#     3. Downloads PDFs to beat-archive/deep-river-agendas/YYYY-MM/
#
#   Video (--include-video or --video-only; off by default — see VIDEO NOTE):
#     4. Queries the regional Cablecast (Tightrope Media Systems) archive
#        directly for the town's own "Deep River Town Meetings" project
#        and downloads any show whose event date falls in the window and
#        has a published VOD
#
...
# Chester/Essex elsewhere in this repo):
#   Hub:     https://www.deepriverct.us/AgendaCenter
#   Search:  https://www.deepriverct.us/AgendaCenter/Search/?term=&CIDs=all
#              &startDate=MM/DD/YYYY&endDate=MM/DD/YYYY&dateRange=Custom&dateSelector=0
#   Agenda:  https://www.deepriverct.us/AgendaCenter/ViewFile/Agenda/_MMDDYYYY-ID
#   Minutes: https://www.deepriverct.us/AgendaCenter/ViewFile/Minutes/_MMDDYYYY-ID
#
# VIDEO NOTE — Unlike Durham, Middlefield, Chester, and Essex elsewhere in
#   this repo (all towns on the same regional Cablecast platform with NO
#   real meeting coverage), Deep River is a genuine hit: the shared
#   Cablecast instance at reflect-vsctv.cablecast.tv carries a dedicated
#   "Deep River Town Meetings" project (ID 7, confirmed directly before
#   writing this script) with ~280 shows going back to 2019 and current
#   through the most recent posting, on a roughly biweekly cadence. Every
#   show in the project is titled generically "Deep River Town Meetings -
#   <date>" (occasionally singular "Town Meeting") rather than by
#   individual board, so this script queries the whole project directly
#   by ID and filters by date — no per-board name matching needed or
#   possible. Because this project ID is specific to Deep River's own
#   town-meeting broadcast (not a shared/ambiguous search term), this
#   video source is reliable enough to treat as primary rather than
#   best-effort, unlike the Cablecast search-by-board-name approach used
#   for Durham/Middlefield/Chester/Essex.
...
# --- Configuration ---
BASE_URL = "https://www.deepriverct.us"
SEARCH_URL = f"{BASE_URL}/AgendaCenter/Search/"
CABLECAST_BASE = "https://reflect-vsctv.cablecast.tv/cablecastapi/v1"
CABLECAST_PROJECT_ID = 7  # "Deep River Town Meetings"
OUTPUT_DIR = "beat-archive/deep-river-agendas"
DAYS_BACK = 4
DAYS_AHEAD = 7
DELAY_SECONDS = 0.8

UA = "Deep-River-CT-Agendas-Downloader/1.0 (journalism research)"
...
# --- (2) Video (reliable — see VIDEO NOTE above) ---

def fetch_cablecast_shows(project_id, page_size=50):
    params = {"pageSize": page_size, "sort": "-eventDate", "project": project_id}
...
def collect_videos(cutoff, future_limit):
    videos = []
    for show in fetch_cablecast_shows(CABLECAST_PROJECT_ID):
        ...
        title = show.get("title") or "Deep River Town Meetings"
        board = title.split(" - ")[0].strip() or "Town Meetings"
        videos.append({
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Download Deep River CT municipal agendas, minutes, and video "
    ...
    parser.add_argument("--board", metavar="NAME",
                        help="Only process boards whose name contains NAME (case-insensitive); "
                             "does not filter video, which isn't broken out by board")
    ...
    parser.add_argument("--include-video", action="store_true",
                        help="Also download the town's Cablecast video recordings")
    ...
    videos = []
    if do_video:
        videos = collect_videos(cutoff, future_limit)
...
#    python3 scripts/download-deep-river-agendas.py --dry-run
#
# 2. Just Board of Selectmen:
#    python3 scripts/download-deep-river-agendas.py --board "Board of Selectmen"
#
# 3. Change the lookback window:
#    python3 scripts/download-deep-river-agendas.py --days 14
#
# 4. Also download Cablecast video (real coverage — see VIDEO NOTE):
#    python3 scripts/download-deep-river-agendas.py --include-video
#
# 5. Run on a schedule (cron — evening):
#    0 21 * * 1-5 cd /path/to/repo && python3 scripts/download-deep-river-agendas.py --include-video
