#!/usr/bin/env python3
# send-north-andover-docs.py
# Run the North Andover MA agenda downloader and email all new files.
#
# NOTE: This is North Andover, MA — not Andover, MA, a separate town.
#
# CONFIGURATION:
...
TO_ADDRESS   = "scott.souza@patch.com"

REPO_DIR   = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
LOG_PATH   = os.path.join(REPO_DIR, "beat-archive", "send-log.txt")
SCRIPT     = os.path.join(REPO_DIR, "scripts", "download-north-andover-agendas.py")
OUTPUT_DIR = os.path.join(REPO_DIR, "beat-archive/north-andover-agendas")
TOWN_NAME  = "North Andover, MA"

ATTACH_EXTENSIONS = {".pdf", ".txt", ".mp4", ".mkv", ".webm", ".m4a", ".mp3", ".mov"}
...
def run_downloader():
    print("Running download-north-andover-agendas.py ...")
...
def send_email(files, downloader_output):
    ...
    body = (
        ...
        "Note: North Andover's Laserfiche minutes have no PDF export "
        "available on the public site, so those come through as .txt "
        "(the same OCR text the public viewer displays), not PDF.\n"
        + skipped_note
        + "\n--- Downloader log ---\n"
        + downloader_output
    )
