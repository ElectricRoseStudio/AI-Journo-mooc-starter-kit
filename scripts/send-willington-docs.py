#!/usr/bin/env python3
# send-willington-docs.py
# Run the Willington CT agenda downloader and email all new files.
#
# CONFIGURATION:
...
SCRIPT     = os.path.join(REPO_DIR, "scripts", "download-willington-agendas.py")
OUTPUT_DIR = os.path.join(REPO_DIR, "beat-archive", "willington-agendas")
...
def run_downloader():
    print("Running download-willington-agendas.py ...")
...
def send_email(files, downloader_output, biz_table=""):
    ...
    subject = (
        f"Willington CT meeting docs — {datetime.date.today().strftime('%B %-d, %Y')} "
    ...
    body = (
        f"Willington CT agenda/minutes download — {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}\n\n"
