#!/usr/bin/env python3
# send-middletown-docs.py
# Run the Middletown CT agenda downloader and email all new files.
#
# CONFIGURATION:
...
SCRIPT     = os.path.join(REPO_DIR, "scripts", "download-middletown-agendas.py")
OUTPUT_DIR = os.path.join(REPO_DIR, "beat-archive", "middletown-agendas")
...
def run_downloader():
    print("Running download-middletown-agendas.py ...")
...
def send_email(files, downloader_output):
    ...
    subject = (
        f"Middletown CT meeting docs — {datetime.date.today().strftime('%B %-d, %Y')} "
    ...
    body = (
        f"Middletown CT agenda/minutes download — {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}\n\n"
