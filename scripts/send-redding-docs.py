#!/usr/bin/env python3
# send-redding-docs.py
# Run the Redding CT agenda downloader and email all new files.
#
# CONFIGURATION:
...
SCRIPT     = os.path.join(REPO_DIR, "scripts", "download-redding-agendas.py")
OUTPUT_DIR = os.path.join(REPO_DIR, "beat-archive", "redding-agendas")
...
def run_downloader():
    print("Running download-redding-agendas.py ...")
...
def send_email(files, downloader_output):
    subject = (
        f"Redding CT meeting docs — {datetime.date.today().strftime('%B %-d, %Y')} "
    ...
    body = (
        f"Redding CT agenda/minutes download — {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}\n\n"
