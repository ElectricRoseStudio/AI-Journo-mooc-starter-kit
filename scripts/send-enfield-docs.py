#!/usr/bin/env python3
# send-enfield-docs.py
# Run the Enfield CT agenda downloader and email all new files.
#
# CONFIGURATION:
...
SCRIPT     = os.path.join(REPO_DIR, "scripts", "download-enfield-agendas.py")
OUTPUT_DIR = os.path.join(REPO_DIR, "beat-archive", "enfield-agendas")
CITY_NAME  = "ENFIELD"

ATTACH_EXTENSIONS = {".pdf", ".mp4", ".mp3"}
...
def run_downloader():
    print("Running download-enfield-agendas.py ...")
...
def send_email(files, downloader_output, biz_table=""):
    ...
    subject = (
        f"Enfield CT meeting docs — {datetime.date.today().strftime('%B %-d, %Y')} "
    ...
    body = (
        f"Enfield CT agenda/minutes download — {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}\n\n"
