#!/usr/bin/env python3
# send-derby-docs.py
# Run the Derby CT agenda downloader and email all new files.
#
# CONFIGURATION:
...
SCRIPT     = os.path.join(REPO_DIR, "scripts", "download-derby-agendas.py")
OUTPUT_DIR = os.path.join(REPO_DIR, "beat-archive", "derby-agendas")
CITY_NAME  = "DERBY"
...
def run_downloader():
    print("Running download-derby-agendas.py ...")
...
def send_email(files, downloader_output, biz_table=""):
    ...
    subject = (
        f"Derby CT meeting docs — {datetime.date.today().strftime('%B %-d, %Y')} "
    ...
    body = (
        f"Derby CT agenda/minutes download — {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}\n\n"
