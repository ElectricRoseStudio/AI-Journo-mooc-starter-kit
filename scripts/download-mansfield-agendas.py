#!/usr/bin/env python3
# download-mansfield-agendas.py
# Download municipal meeting agendas and minutes from Mansfield CT for meetings
...
#
# USAGE:
#   python3 scripts/download-mansfield-agendas.py [options]
#
# REQUIREMENTS:
...
#
# WHAT IT DOES:
#   1. Fetches the Mansfield CT Agenda Center hub page to discover all board
#      category IDs (~60 boards)
...
#   4. Downloads PDFs to beat-archive/mansfield-agendas/YYYY-MM/
...
#   7. Appends a download log to beat-archive/mansfield-agendas/download-log.txt
#
# SITE STRUCTURE:
#   AgendaCenter (CivicPlus CivicEngage):
#     Hub:    https://www.mansfieldct.gov/agendacenter
...
#
#   YouTube channel: UCFopiiyApXfwNBHnmwJmmog
#     RSS: https://www.youtube.com/feeds/videos.xml?channel_id=UCFopiiyApXfwNBHnmwJmmog
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Download Mansfield CT municipal agendas, minutes, and meeting recordings "
...
#    python3 scripts/download-mansfield-agendas.py --dry-run
#
# 2. Narrow to one board:
#    python3 scripts/download-mansfield-agendas.py --board "Town Council"
#
# 3. PDFs only (no video downloads):
#    python3 scripts/download-mansfield-agendas.py --docs-only
#
# 4. Videos only:
#    python3 scripts/download-mansfield-agendas.py --videos-only
#
# 5. Agendas only (skip minutes):
#    python3 scripts/download-mansfield-agendas.py --no-minutes
#
# 6. Change the lookback window:
#    python3 scripts/download-mansfield-agendas.py --days 7
#
# 7. Save files somewhere else:
#    python3 scripts/download-mansfield-agendas.py --output-dir ~/Downloads/mansfield
#
# 8. Run on a schedule (cron — 7 AM daily):
#    0 7 * * * cd /path/to/repo && python3 scripts/download-mansfield-agendas.py
#
# NOTES:
#   - Mansfield CT uses CivicPlus CivicEngage. All board rows are embedded
...
#       /AgendaCenter/ViewFile/Agenda/_05072026-17497 → May 7, 2026
#   - YouTube videos are sourced from channel UCFopiiyApXfwNBHnmwJmmog via
