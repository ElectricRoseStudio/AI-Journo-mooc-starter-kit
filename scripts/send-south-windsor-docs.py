#!/usr/bin/env python3
# send-south-windsor-docs.py
# Run the South Windsor CT agenda downloader and email all new files.
#
# CONFIGURATION:
...
SCRIPT     = os.path.join(REPO_DIR, "scripts", "download-south-windsor-agendas.py")
OUTPUT_DIR = os.path.join(REPO_DIR, "beat-archive", "south-windsor-agendas")
...
def run_downloader():
    print("Running download-south-windsor-agendas.py ...")
...
def send_email(files, downloader_output, biz_table=""):
    ...
    subject = (
        f"South Windsor CT meeting docs — {datetime.date.today().strftime('%B %-d, %Y')} "
    ...
    body = (
        f"South Windsor CT agenda/minutes download — {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}\n\n"
