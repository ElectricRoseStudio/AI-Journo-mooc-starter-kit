#!/usr/bin/env python3
# send-new-canaan-docs.py
# Run the New Canaan CT agenda downloader and email all new files.
#
# CONFIGURATION:
...
SCRIPT     = os.path.join(REPO_DIR, "scripts", "download-new-canaan-agendas.py")
OUTPUT_DIR = os.path.join(REPO_DIR, "beat-archive", "new-canaan-agendas")
...
def run_downloader():
    print("Running download-new-canaan-agendas.py ...")
...
def send_email(files, downloader_output):
    ...
    subject = (
        f"New Canaan CT meeting docs — {datetime.date.today().strftime('%B %-d, %Y')} "
    ...
    body = (
        f"New Canaan CT agenda/minutes download — {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}\n\n"
