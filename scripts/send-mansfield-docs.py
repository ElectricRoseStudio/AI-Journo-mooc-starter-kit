#!/usr/bin/env python3
# send-mansfield-docs.py
# Run the Mansfield CT agenda downloader and email all new files.
#
# CONFIGURATION:
...
SCRIPT     = os.path.join(REPO_DIR, "scripts", "download-mansfield-agendas.py")
OUTPUT_DIR = os.path.join(REPO_DIR, "beat-archive", "mansfield-agendas")
CITY_NAME  = "MANSFIELD"
...
def run_downloader():
    print("Running download-mansfield-agendas.py ...")
...
def send_email(files, downloader_output, biz_table=""):
    ...
    subject = (
        f"Mansfield CT meeting docs — {datetime.date.today().strftime('%B %-d, %Y')} "
    ...
    body = (
        f"Mansfield CT agenda/minutes download — {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}\n\n"
