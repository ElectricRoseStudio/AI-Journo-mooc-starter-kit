#!/usr/bin/env python3
# download-chester-agendas.py
# Download municipal meeting agendas, minutes, and (best-effort) video
# recordings from the Chester, CT Agenda Center for meetings within the
# past N days (and up to M days ahead).
#
# USAGE:
#   python3 scripts/download-chester-agendas.py [options]
#
# REQUIREMENTS:
...
#     1. Fetches the Chester CT Agenda Center search endpoint with a
...
#     3. Downloads PDFs to beat-archive/chester-agendas/YYYY-MM/
#
...
#   Hub:     https://www.chesterct.org/AgendaCenter
#   Search:  https://www.chesterct.org/AgendaCenter/Search/?term=&CIDs=all
#              &startDate=MM/DD/YYYY&endDate=MM/DD/YYYY&dateRange=Custom&dateSelector=0
#   Agenda:  https://www.chesterct.org/AgendaCenter/ViewFile/Agenda/_MMDDYYYY-ID
#   Minutes: https://www.chesterct.org/AgendaCenter/ViewFile/Minutes/_MMDDYYYY-ID
#
# VIDEO NOTE — No reliable per-meeting video source was found for
#   Chester. Checked directly before writing this script:
#     - No YouTube/Vimeo/Facebook-Live/PEG link anywhere on chesterct.org.
#     - The regional Cablecast (Tightrope Media Systems) instance at
#       reflect-vsctv.cablecast.tv — the same platform already used for
#       Durham and Middlefield elsewhere in this repo — carries Chester
#       content, but it's all community programming (Chester Folk Fest,
#       "Chester First Selectwoman Update" talk segments, Chester Lights,
#       Tractor Parade) — no actual board-meeting recordings.
#     - A YouTube search for "Chester CT Board of Selectmen meeting"
#       returns nothing from Chester at all (unrelated towns only).
#   This is the same gap already found for Durham and Middlefield — video
#   search is included anyway for completeness and left OFF by default
#   (--include-video/--video-only to try it) — expect it to find nothing
#   until/unless Chester starts recording meetings somewhere discoverable.
...
# --- Configuration ---
BASE_URL = "https://www.chesterct.org"
SEARCH_URL = f"{BASE_URL}/AgendaCenter/Search/"
CABLECAST_BASE = "https://reflect-vsctv.cablecast.tv/cablecastapi/v1"
OUTPUT_DIR = "beat-archive/chester-agendas"
DAYS_BACK = 4
DAYS_AHEAD = 7
DELAY_SECONDS = 0.8

UA = "Chester-CT-Agendas-Downloader/1.0 (journalism research)"
...
def collect_videos(cutoff, future_limit, board_filter, known_boards):
    ...
    for board in sorted(boards):
        for show in fetch_cablecast_shows(search=f"Chester {board}"):
            if show["id"] in seen_show_ids:
                continue
            title = show.get("title") or ""
            if "chester" not in title.lower() or board.lower() not in title.lower():
                continue
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Download Chester CT municipal agendas, minutes, and (best-effort) "
...
#    python3 scripts/download-chester-agendas.py --dry-run
#
# 2. Just Board of Selectmen:
#    python3 scripts/download-chester-agendas.py --board "Board of Selectmen"
#
# 3. Change the lookback window:
#    python3 scripts/download-chester-agendas.py --days 14
#
# 4. Also try Cablecast video search (see VIDEO NOTE — expect little/nothing):
#    python3 scripts/download-chester-agendas.py --include-video
#
# 5. Run on a schedule (cron — evening):
#    0 20 * * 1-5 cd /path/to/repo && python3 scripts/download-chester-agendas.py
