#!/usr/bin/env python3
# send-east-windsor-docs.py
# Run the East Windsor CT agenda downloader and email all new files.
#
# CONFIGURATION:
...
SCRIPT     = os.path.join(REPO_DIR, "scripts", "download-east-windsor-agendas.py")
OUTPUT_DIR = os.path.join(REPO_DIR, "beat-archive", "east-windsor-agendas")
...
def run_downloader():
    print("Running download-east-windsor-agendas.py ...")
...
def send_email(files, downloader_output):
    ...
    subject = (
        f"East Windsor CT meeting docs — {datetime.date.today().strftime('%B %-d, %Y')} "
    ...
    body = (
        f"East Windsor CT agenda/minutes download — {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}\n\n"
