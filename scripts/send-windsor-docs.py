#!/usr/bin/env python3
# send-windsor-docs.py
# Run the Windsor CT agenda downloader and email all new files.
#
# CONFIGURATION:
...
SCRIPT     = os.path.join(REPO_DIR, "scripts", "download-windsor-agendas.py")
OUTPUT_DIR = os.path.join(REPO_DIR, "beat-archive", "windsor-agendas")
CITY_NAME  = "WINDSOR"
...
def run_downloader():
    print("Running download-windsor-agendas.py ...")
...
def send_email(files, downloader_output, biz_table=""):
    ...
    subject = (
        f"Windsor CT meeting docs — {datetime.date.today().strftime('%B %-d, %Y')} "
    ...
    body = (
        f"Windsor CT agenda/minutes download — {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}\n\n"
