#!/usr/bin/env python3
# send-peabody-docs.py
# Run the Peabody MA agenda downloader and email all new files.
#
# CONFIGURATION:
...
SCRIPT     = os.path.join(REPO_DIR, "scripts", "download-peabody-agendas.py")
OUTPUT_DIR = os.path.join(REPO_DIR, "beat-archive/peabody-agendas")
TOWN_NAME  = "Peabody, MA"
...
def run_downloader():
    print("Running download-peabody-agendas.py ...")
...
def send_email(files, downloader_output):
    ...
    body = (
        ...
        "Note: Parks Commission has no minutes source available online — "
        "agenda coverage only (see the downloader script for details).\n"
        + skipped_note
        + "\n--- Downloader log ---\n"
        + downloader_output
    )
