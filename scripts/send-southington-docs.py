#!/usr/bin/env python3
# send-southington-docs.py
# Run the Southington CT agenda downloader and email all new files.
#
# CONFIGURATION:
...
SCRIPT     = os.path.join(REPO_DIR, "scripts", "download-southington-agendas.py")
OUTPUT_DIR = os.path.join(REPO_DIR, "beat-archive", "southington-agendas")
...
def run_downloader():
    print("Running download-southington-agendas.py ...")
...
def send_email(files, downloader_output):
    subject = (
        f"Southington CT meeting docs — {datetime.date.today().strftime('%B %-d, %Y')} "
    ...
    body = (
        f"Southington CT agenda/minutes download — {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}\n\n"
