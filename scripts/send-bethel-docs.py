#!/usr/bin/env python3
# send-bethel-docs.py
# Run the Bethel CT agenda downloader and email all new files.
#
# CONFIGURATION:
...
SCRIPT     = os.path.join(REPO_DIR, "scripts", "download-bethel-agendas.py")
OUTPUT_DIR = os.path.join(REPO_DIR, "beat-archive", "bethel-agendas")
CITY_NAME  = "BETHEL"
...
def run_downloader():
    print("Running download-bethel-agendas.py ...")
...
def send_email(files, downloader_output, biz_table=""):
    ...
    subject = (
        f"Bethel CT meeting docs — {datetime.date.today().strftime('%B %-d, %Y')} "
    ...
    body = (
        f"Bethel CT agenda/minutes download — {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}\n\n"
