#!/usr/bin/env python3
# send-milford-docs.py
# Run the Milford CT agenda downloader and email all new files.
#
# CONFIGURATION:
...
SCRIPT     = os.path.join(REPO_DIR, "scripts", "download-milford-agendas.py")
OUTPUT_DIR = os.path.join(REPO_DIR, "beat-archive", "milford-agendas")
CITY_NAME  = "MILFORD"
...
def run_downloader():
    print("Running download-milford-agendas.py ...")
...
def send_email(files, downloader_output, biz_table=""):
    ...
    subject = (
        f"Milford CT meeting docs — {datetime.date.today().strftime('%B %-d, %Y')} "
    ...
    body = (
        f"Milford CT agenda/minutes download — {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}\n\n"
