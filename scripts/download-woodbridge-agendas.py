#!/usr/bin/env python3
# download-woodbridge-agendas.py
# Download municipal meeting agendas, minutes, and video recordings from
# the Woodbridge, CT Agenda Center for meetings within the past N days
# (and up to M days ahead).
#
# USAGE:
#   python3 scripts/download-woodbridge-agendas.py [options]
#
# REQUIREMENTS:
...
#     1. Fetches the Woodbridge CT Agenda Center search endpoint with a
...
#     3. Downloads PDFs to beat-archive/woodbridge-agendas/YYYY-MM/
#
#   Video (--include-video or --video-only; off by default):
#     4. Downloads recent videos from the WGATV79 (Woodbridge Government
#        Access TV) YouTube channel using yt-dlp — this channel posts real,
#        current per-meeting recordings (titled e.g. "Fire Commission-
#        7/20/2026"), confirmed directly before writing this script.
#
# SITE STRUCTURE (CivicPlus CivicEngage "Agenda Center", same platform as
# Clinton/Haddam/Salem/Swampscott elsewhere in this repo):
#   Hub:     https://www.woodbridgect.org/AgendaCenter
#   Search:  https://www.woodbridgect.org/AgendaCenter/Search/?term=&CIDs=all
#              &startDate=MM/DD/YYYY&endDate=MM/DD/YYYY&dateRange=Custom&dateSelector=0
#   Agenda:  https://www.woodbridgect.org/AgendaCenter/ViewFile/Agenda/_MMDDYYYY-ID
#   Minutes: https://www.woodbridgect.org/AgendaCenter/ViewFile/Minutes/_MMDDYYYY-ID
#   YouTube: https://www.youtube.com/@wgatv79
...
# --- Configuration ---
BASE_URL = "https://www.woodbridgect.org"
SEARCH_URL = f"{BASE_URL}/AgendaCenter/Search/"
YOUTUBE_CHANNEL = "https://www.youtube.com/@wgatv79"
OUTPUT_DIR = "beat-archive/woodbridge-agendas"
DAYS_BACK = 4
DAYS_AHEAD = 7
DELAY_SECONDS = 0.8
...
UA = "Woodbridge-CT-Agendas-Downloader/1.0 (journalism research)"
...
def download_channel_videos(channel_url, output_dir, date_after, dry_run=False):
    ...
    cmd = [
        ...
        "--dateafter", date_str,
        # WGATV79 lists not-yet-started scheduled livestreams in its channel
        # feed ("This live event will begin in N days"), which yt-dlp can't
        # extract. Without --ignore-errors, hitting one of these aborts the
        # whole run before it reaches any real, downloadable video after it.
        "--ignore-errors",
        # No --break-match-filters here (unlike some other channel-based
        # downloaders in this repo): those scheduled-livestream placeholders
        # have no upload_date, so a date-based break filter treats the first
        # one as "too old" and stops the walk right there — before ever
        # reaching the real videos beneath it. --playlist-end below already
        # bounds the walk cheaply enough without needing an early-break filter.
        "--playlist-end", "20",
        "--sleep-requests", "0.75",
        "--sleep-interval", "10",
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Download Woodbridge CT municipal agendas, minutes, and video "
    ...
    parser.add_argument("--include-video", action="store_true", help="Also download WGATV79 YouTube channel video")
...
#    python3 scripts/download-woodbridge-agendas.py --dry-run
#
# 2. Just Board of Selectmen:
#    python3 scripts/download-woodbridge-agendas.py --board "Board of Selectmen"
#
# 3. PDFs only (no video — the default; --include-video/--video-only turn it on):
#    python3 scripts/download-woodbridge-agendas.py
#
# 4. Video only:
#    python3 scripts/download-woodbridge-agendas.py --video-only
#
# 5. Change the lookback window:
#    python3 scripts/download-woodbridge-agendas.py --days 14
#
# 6. Run on a schedule (cron — evening):
#    0 19 * * 1-5 cd /path/to/repo && python3 scripts/download-woodbridge-agendas.py --include-video
