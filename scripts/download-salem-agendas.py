#!/usr/bin/env python3
# download-salem-agendas.py
# Download municipal meeting agendas, minutes, and video recordings from
# the Salem MA Agenda Center for meetings within the past N days (and
# up to 7 days ahead).
#
# USAGE:
#   python3 scripts/download-salem-agendas.py [options]
#
# REQUIREMENTS:
...
#     1. Fetches the Salem MA Agenda Center search endpoint with a date
...
#     3. Downloads PDFs to beat-archive/salem-agendas/YYYY-MM/
#
#   Video (--include-video or --video-only):
#     4. Extracts each meeting's video link, if any, from the row's
#        <td class="media"> cell and downloads it via yt-dlp
#
# SITE STRUCTURE (CivicPlus CivicEngage "Agenda Center", same platform as
# most other MA towns in this repo):
#   Hub:     https://www.salemma.gov/AgendaCenter
#   Search:  https://www.salemma.gov/AgendaCenter/Search/?term=&CIDs=all
#              &startDate=MM/DD/YYYY&endDate=MM/DD/YYYY&dateRange=Custom&dateSelector=0
#   Agenda:  https://www.salemma.gov/AgendaCenter/ViewFile/Agenda/_MMDDYYYY-ID
#   Minutes: https://www.salemma.gov/AgendaCenter/ViewFile/Minutes/_MMDDYYYY-ID
#
#   (salem.com, the obvious first guess, 301-redirects to salemma.gov —
#   confirmed directly, not assumed.)
#
#   Video: one link per meeting (if any) in <td class="media">, unlike
#   most other AgendaCenter towns in this repo where that cell is either
#   empty or a live-meeting join link with no archival value. Salem
#   actually posts real recordings there, from two different hosts,
#   confirmed directly on a live 2-month window:
#     - youtube.com/youtu.be — used by School Committee
#     - {tenant}.sharepoint.com/:v:/... "share" links — used by Zoning
#       Board of Appeals (labeled inconsistently as "audio" or "videos"
#       in the page's own CSS class, but both are real recordings).
#       Confirmed with a live yt-dlp --simulate run that these need no
#       authentication (SharePoint's own "anyone with the link" sharing)
#       and resolve through yt-dlp's built-in SharePoint extractor to a
#       real HLS manifest, not a login wall.
#   Both host patterns are downloaded via yt-dlp; anything else (e.g. a
#   Zoom/Google Meet join link, seen on other MA AgendaCenter towns in
#   this repo) is left alone since it isn't a recording.
#
# COVERAGE: Both School Committee (Board of Education) and Park &
# Recreation Commission (Parks & Recreation) are native Agenda Center
# categories — confirmed directly with real, current postings for both
...
# --- Configuration ---
BASE_URL = "https://www.salemma.gov"
SEARCH_URL = f"{BASE_URL}/AgendaCenter/Search/"
OUTPUT_DIR = "beat-archive/salem-agendas"
DAYS_BACK = 4
DAYS_AHEAD = 7
DELAY_SECONDS = 0.8

UA = "Salem-MA-Agendas-Downloader/1.0 (journalism research)"
...
# Real recordings confirmed on this site (see platform notes above);
# anything else in the media cell (Zoom/Meet join links etc.) is skipped.
_VIDEO_HOST_RE = re.compile(
    r"https?://(?:www\.)?(youtube\.com|youtu\.be|[\w-]+\.sharepoint\.com)/", re.IGNORECASE
)
...
class AgendaParser(html.parser.HTMLParser):
    """
    Single-pass parser for the Salem CivicPlus Agenda Center search
    ...
    def handle_starttag(self, tag, attrs):
        ...
        if tag == "h2":
            ...
            # PDF (confirmed on this same CivicPlus platform for several
            # other MA towns in this repo) — strip any query string
            # before matching, but keep the original for video links
            # (SharePoint links carry a required "?e=..." token).
            path_only = href.split("?", 1)[0]
            lower = path_only.lower()
            if "/agendacenter/viewfile/agenda/" in lower:
                if self._agenda_url is None:
                    self._agenda_url = path_only
            elif "/agendacenter/viewfile/minutes/" in lower:
                if self._minutes_url is None:
                    self._minutes_url = path_only
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Download Salem MA municipal agendas, minutes, and video recordings "
...
#    python3 scripts/download-salem-agendas.py --dry-run
#
# 2. Narrow to one board:
#    python3 scripts/download-salem-agendas.py --board "School Committee"
#
# 3. Download documents AND video recordings:
#    python3 scripts/download-salem-agendas.py --include-video
#
# 4. Download only video recordings:
#    python3 scripts/download-salem-agendas.py --video-only
#
# 5. Change the lookback window:
#    python3 scripts/download-salem-agendas.py --days 14
#
# 6. Run on a schedule (cron — evening):
#    0 19 * * 1-5 cd /path/to/repo && python3 scripts/download-salem-agendas.py
