#!/usr/bin/env python3
# send-tolland-docs.py
# Run the Tolland CT agenda downloader and email all new files.
#
# CONFIGURATION:
...
SCRIPT     = os.path.join(REPO_DIR, "scripts", "download-tolland-agendas.py")
OUTPUT_DIR = os.path.join(REPO_DIR, "beat-archive", "tolland-agendas")
CITY_NAME  = "TOLLAND"
...
def run_downloader():
    print("Running download-tolland-agendas.py ...")
...
def send_email(files, downloader_output, biz_table=""):
    ...
    subject = (
        f"Tolland CT meeting docs — {datetime.date.today().strftime('%B %-d, %Y')} "
    ...
    body = (
        f"Tolland CT agenda/minutes download — {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}\n\n"
