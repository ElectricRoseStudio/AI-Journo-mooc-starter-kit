#!/usr/bin/env python3
# send-naugatuck-docs.py
# Run the Naugatuck CT agenda downloader and email all new files.
#
# CONFIGURATION:
...
TO_ADDRESS   = "rj.scofield@patch.com"

REPO_DIR   = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
LOG_PATH   = os.path.join(REPO_DIR, "beat-archive", "send-log.txt")
SCRIPT     = os.path.join(REPO_DIR, "scripts", "download-naugatuck-agendas.py")
OUTPUT_DIR = os.path.join(REPO_DIR, "beat-archive", "naugatuck-agendas")

ATTACH_EXTENSIONS = {".pdf", ".mp4", ".mp3", ".m4a", ".wav"}
...
def run_downloader():
    print("Running download-naugatuck-agendas.py ...")
...
def send_email(files, downloader_output):
    ...
    subject = (
        f"Naugatuck CT meeting docs — {datetime.date.today().strftime('%B %-d, %Y')} "
    ...
    body = (
        f"Naugatuck CT agenda/minutes download — {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}\n\n"
