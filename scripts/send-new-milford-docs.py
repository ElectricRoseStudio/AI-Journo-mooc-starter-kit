#!/usr/bin/env python3
# send-new-milford-docs.py
# Run the New Milford CT agenda downloader and email all new files.
#
# CONFIGURATION:
...
SCRIPT     = os.path.join(REPO_DIR, "scripts", "download-new-milford-agendas.py")
OUTPUT_DIR = os.path.join(REPO_DIR, "beat-archive/new-milford-agendas")
...
def run_downloader():
    print("Running download-new-milford-agendas.py ...")
...
def send_email(files, downloader_output, biz_table=""):
    ...
    body = (
        f"{CITY_NAME.title()} CT agenda/minutes download — {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}\n\n"
