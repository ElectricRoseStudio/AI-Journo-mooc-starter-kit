#!/usr/bin/env python3
# send-avon-docs.py
# Run the Avon CT agenda downloader and email all new files.
#
# CONFIGURATION:
...
SCRIPT     = os.path.join(REPO_DIR, "scripts", "download-avon-agendas.py")
OUTPUT_DIR = os.path.join(REPO_DIR, "beat-archive", "avon-agendas")
...
def run_downloader():
    print("Running download-avon-agendas.py ...")
...
def send_email(files, downloader_output):
    subject = (
        f"Avon CT meeting docs — {datetime.date.today().strftime('%B %-d, %Y')} "
    ...
    body = (
        f"Avon CT agenda/minutes download — {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}\n\n"
