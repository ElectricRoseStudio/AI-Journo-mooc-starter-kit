#!/usr/bin/env python3
# send-bloomfield-docs.py
# Run the Bloomfield CT agenda downloader and email all new files.
#
# CONFIGURATION:
...
SCRIPT     = os.path.join(REPO_DIR, "scripts", "download-bloomfield-agendas.py")
OUTPUT_DIR = os.path.join(REPO_DIR, "beat-archive", "bloomfield-agendas")
...
def run_downloader():
    print("Running download-bloomfield-agendas.py ...")
...
def send_email(files, downloader_output):
    subject = (
        f"Bloomfield CT meeting docs — {datetime.date.today().strftime('%B %-d, %Y')} "
    ...
    body = (
        f"Bloomfield CT agenda/minutes download — {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}\n\n"
