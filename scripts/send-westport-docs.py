#!/usr/bin/env python3
# send-westport-docs.py
# Run the Westport CT agenda downloader and email all new files.
#
# CONFIGURATION:
...
SCRIPT     = os.path.join(REPO_DIR, "scripts", "download-westport-agendas.py")
OUTPUT_DIR = os.path.join(REPO_DIR, "beat-archive", "westport-agendas")
...
def run_downloader():
    print("Running download-westport-agendas.py ...")
...
def send_email(files, downloader_output):
    subject = (
        f"Westport CT meeting docs — {datetime.date.today().strftime('%B %-d, %Y')} "
    ...
    body = (
        f"Westport CT agenda/minutes download — {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}\n\n"
