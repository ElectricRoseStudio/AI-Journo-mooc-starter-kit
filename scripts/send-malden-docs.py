#!/usr/bin/env python3
# send-malden-docs.py
# Run the Malden MA agenda downloader and email all new files.
#
# CONFIGURATION:
...
TO_ADDRESS   = "benjamin.pierce@patch.com"

REPO_DIR   = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
LOG_PATH   = os.path.join(REPO_DIR, "beat-archive", "send-log.txt")
SCRIPT     = os.path.join(REPO_DIR, "scripts", "download-malden-agendas.py")
OUTPUT_DIR = os.path.join(REPO_DIR, "beat-archive/malden-agendas")
TOWN_NAME  = "Malden, MA"
...
# .url Internet Shortcut files (SharePoint/Zoom/Teams/tinyurl video links
# the downloader can't fetch directly) are small and always safe to attach —
# skip the size-budget dance for them so the editor always gets the link.
LINK_EXTENSIONS = {".url"}
...
def run_downloader():
    print("Running download-malden-agendas.py ...")
...
def collect_recent_files(hours=24):
    ...
    for root, _, files in os.walk(OUTPUT_DIR):
        for fname in sorted(files):
            ...
            if ext not in ATTACH_EXTENSIONS and ext not in LINK_EXTENSIONS:
                continue
...
def send_email(files, downloader_output):
    ...
    body = (
        ...
        "Video note: Malden posts video via YouTube (attached as .mp4), or via "
        "SharePoint/Zoom/Teams/tinyurl links the downloader can't fetch directly "
        "(attached as a .url shortcut — open it to view/download from the source).\n"
        + skipped_note
        + "\n--- Downloader log ---\n"
        + downloader_output
    )
