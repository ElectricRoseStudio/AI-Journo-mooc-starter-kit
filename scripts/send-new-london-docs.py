#!/usr/bin/env python3
# send-new-london-docs.py
# Run the New London CT agenda downloader and email all new files.
#
# CONFIGURATION:
...
TO_ADDRESS   = "chris.rhatigan@patch.com"

REPO_DIR   = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
LOG_PATH   = os.path.join(REPO_DIR, "beat-archive", "send-log.txt")
SCRIPT     = os.path.join(REPO_DIR, "scripts", "download-new-london-agendas.py")
OUTPUT_DIR = os.path.join(REPO_DIR, "beat-archive", "new-london-agendas")
CITY_NAME  = "NEW LONDON"

ATTACH_EXTENSIONS = {".pdf", ".mp3"}
...
def run_downloader():
    print("Running download-new-london-agendas.py ...")
...
def send_email(files, downloader_output, biz_table=""):
    ...
    subject = (
        f"New London CT meeting docs — {datetime.date.today().strftime('%B %-d, %Y')} "
    ...
    body = (
        f"New London CT agenda/minutes download — {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}\n\n"
