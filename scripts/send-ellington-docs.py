#!/usr/bin/env python3
# send-ellington-docs.py
# Run the Ellington CT agenda downloader and email all new files.
#
# CONFIGURATION:
...
SCRIPT     = os.path.join(REPO_DIR, "scripts", "download-ellington-agendas.py")
OUTPUT_DIR = os.path.join(REPO_DIR, "beat-archive", "ellington-agendas")
...
def run_downloader():
    print("Running download-ellington-agendas.py ...")
...
def send_email(files, downloader_output):
    ...
    subject = (
        f"Ellington CT meeting docs — {datetime.date.today().strftime('%B %-d, %Y')} "
    ...
    body = (
        f"Ellington CT agenda/minutes download — {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}\n\n"
