#!/usr/bin/env python3
# send-monroe-docs.py
# Run the Monroe CT agenda downloader and email all new files.
#
# CONFIGURATION:
...
SCRIPT     = os.path.join(REPO_DIR, "scripts", "download-monroe-agendas.py")
OUTPUT_DIR = os.path.join(REPO_DIR, "beat-archive", "monroe-agendas")
...
def run_downloader():
    print("Running download-monroe-agendas.py ...")
...
def send_email(files, downloader_output):
    subject = (
        f"Monroe CT meeting docs — {datetime.date.today().strftime('%B %-d, %Y')} "
    ...
    body = (
        f"Monroe CT agenda/minutes download — {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}\n\n"
