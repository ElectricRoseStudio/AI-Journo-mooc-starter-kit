#!/usr/bin/env python3
# download-petaluma-agendas.py
# Download Petaluma, CA municipal agendas, minutes, and recording links
# for meetings posted in the past N days.
#
# USAGE:
#   python3 scripts/download-petaluma-agendas.py [options]
#
# REQUIREMENTS:
...
#   5. Downloads agenda and/or minutes PDFs (Azure Blob Storage via signed URL)
#   6. Saves Swagit video recording shortcuts (.url files) for meetings with video
...
#   Videos:     Swagit platform (petalumaca.new.swagit.com); videoUrl field in JSON
#   Legacy:     Pre-PrimeGov recordings on Granicus:
#               https://petaluma.granicus.com/ViewPublisher.php?view_id=3  (City Council)
#               https://petaluma.granicus.com/ViewPublisher.php?view_id=5  (Planning Commission)
...
BASE_URL = "https://cityofpetaluma.primegov.com"
PORTAL_URL = f"{BASE_URL}/public/portal/meetings"
...
OUTPUT_DIR = "beat-archive/petaluma-agendas"
DAYS_BACK = 4
DELAY_SECONDS = 1.0
...
def download_document(opener, template_id, output_type, dest_path):
    """Download a compiled document by templateId; returns True on success."""
    url = (f"{COMPILED_DOC_URL}?meetingTemplateId={template_id}"
           f"&compileOutputType={output_type}")
    req = urllib.request.Request(url, headers={
        "User-Agent": UA,
...
def download_direct(opener, url, dest_path):
    """Download a document from a direct URL (doc.link field); returns True on success."""
...
def get_docs_for_meeting(meeting, want_agendas, want_minutes):
    """
    Return list of (doc_type, template_id_or_url, is_direct_link) for PDF docs only.
    Picks the first Agenda and first Minutes document found.
    """
    agenda_found = minutes_found = False
    docs = []
    for doc in meeting.get("documentList", []):
        if doc.get("compileOutputType") != 1:
            continue  # skip HTML documents
        name = doc.get("templateName", "")
        direct_link = doc.get("link")
        template_id = doc.get("templateId")

        if want_agendas and not agenda_found and _AGENDA_RE.search(name):
            key = direct_link if direct_link else template_id
            docs.append(("agenda", key, bool(direct_link)))
            agenda_found = True
        elif want_minutes and not minutes_found and _MINUTES_RE.search(name):
            key = direct_link if direct_link else template_id
            docs.append(("minutes", key, bool(direct_link)))
            minutes_found = True

        if agenda_found and minutes_found:
            break
    return docs
...
def main():
    parser = argparse.ArgumentParser(
        description="Download Petaluma, CA municipal agendas and minutes "
    ...
    if args.dry_run:
        print(f"\n{'Date':<12} {'Committee':<40} {'Docs':<25} {'Video'}")
        print("-" * 90)
        for d, cname, m in all_meetings:
            docs = get_docs_for_meeting(m, want_agendas, want_minutes)
            doc_str = ", ".join(t for t, _, _ in docs) or "—"
            has_vid = ("yes" if (not args.no_video
                                 and m.get("isShowVideoIcon")
                                 and m.get("videoUrl"))
                       else "no")
            print(f"{str(d):<12} {cname[:39]:<40} {doc_str:<25} {has_vid}")
    ...
    for d, cname, m in all_meetings:
        ...
        for doc_type, key, is_direct in get_docs_for_meeting(m, want_agendas,
                                                              want_minutes):
            ...
            if is_direct:
                ok = download_direct(opener, key, dest)
            else:
                ok = download_document(opener, key, 1, dest)
            ...
            if ok:
                ...
                log_lines.append(
                    ...
                    f"{'direct:' + key if is_direct else 'template:' + str(key)}")
                if os.path.exists(dest):
                    os.remove(dest)
...
#    python3 scripts/download-petaluma-agendas.py --dry-run
#
# 2. Narrow to one board:
#    python3 scripts/download-petaluma-agendas.py --board "City Council"
#    python3 scripts/download-petaluma-agendas.py --board "Planning Commission"
#
# 3. Change the lookback window:
#    python3 scripts/download-petaluma-agendas.py --days 7
#
# 4. Save files elsewhere:
#    python3 scripts/download-petaluma-agendas.py --output-dir ~/Downloads/petaluma
#
# 5. Agendas only (skip minutes):
#    python3 scripts/download-petaluma-agendas.py --no-minutes
#
# 6. Skip recording shortcuts:
#    python3 scripts/download-petaluma-agendas.py --no-video
#
# 7. Run on a schedule (cron — 7 AM daily):
#    0 7 * * * cd /path/to/repo && python3 scripts/download-petaluma-agendas.py
#
# NOTE ON RECORDINGS:
#   Current recordings are on Swagit (petalumaca.new.swagit.com). The script saves
#   .url shortcuts pointing to each meeting's Swagit page. Pre-PrimeGov recordings
#   are archived on Granicus:
#     City Council:        https://petaluma.granicus.com/ViewPublisher.php?view_id=3
#     Planning Commission: https://petaluma.granicus.com/ViewPublisher.php?view_id=5
