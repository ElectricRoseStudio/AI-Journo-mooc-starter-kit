...
def get_video_upload_date(video_id):
    ...
    try:
        result = subprocess.run(
            ["yt-dlp", "--js-runtimes", YT_DLP_NODE, "--no-playlist", "--no-update", "--print", "upload_date",
...
def download_video(video_id, title, upload_date, dry_run):
    ...
    try:
        ...
        print(f"    ERROR: yt-dlp timed out downloading '{title}' — partial file kept, will resume next run")
        return False
    except Exception as e:
...
def main():
    ...
    for vid_id in video_ids:
        ...
        try:
            r = subprocess.run(
                ["yt-dlp", "--js-runtimes", YT_DLP_NODE, "--no-playlist", "--no-update", "--print", "title",
