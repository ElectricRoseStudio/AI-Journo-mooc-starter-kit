#!/usr/bin/env python3
# send-stamford-docs.py
# Run the Stamford CT agenda downloader and email all new files.
#
# CONFIGURATION:
...
SCRIPT     = os.path.join(REPO_DIR, "scripts", "download-stamford-agendas.py")
OUTPUT_DIR = os.path.join(REPO_DIR, "beat-archive", "stamford-agendas")
...
def run_downloader():
    print("Running download-stamford-agendas.py --include-video ...")
...
def send_email(files, downloader_output):
    ...
    subject = (
        f"Stamford CT meeting docs — {datetime.date.today().strftime('%B %-d, %Y')} "
    ...
    body = (
        f"Stamford CT agenda/minutes download — {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}\n\n"
