#!/usr/bin/env python3
# send-easton-docs.py
# Run the Easton CT agenda downloader and email all new files.
#
# CONFIGURATION:
...
SCRIPT     = os.path.join(REPO_DIR, "scripts", "download-easton-agendas.py")
OUTPUT_DIR = os.path.join(REPO_DIR, "beat-archive", "easton-agendas")
...
def run_downloader():
    print("Running download-easton-agendas.py ...")
...
def send_email(files, downloader_output):
    subject = (
        f"Easton CT meeting docs — {datetime.date.today().strftime('%B %-d, %Y')} "
    ...
    body = (
        f"Easton CT agenda/minutes download — {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}\n\n"
