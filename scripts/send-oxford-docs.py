#!/usr/bin/env python3
# send-oxford-docs.py
# Run the Oxford CT agenda downloader and email all new files.
#
# CONFIGURATION:
...
SCRIPT     = os.path.join(REPO_DIR, "scripts", "download-oxford-agendas.py")
OUTPUT_DIR = os.path.join(REPO_DIR, "beat-archive", "oxford-agendas")
...
def run_downloader():
    print("Running download-oxford-agendas.py ...")
...
def send_email(files, downloader_output):
    ...
    subject = (
        f"Oxford CT meeting docs — {datetime.date.today().strftime('%B %-d, %Y')} "
    ...
    body = (
        f"Oxford CT agenda/minutes download — {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}\n\n"
