#!/usr/bin/env python3
# download-essex-agendas.py
# Download municipal meeting agendas, minutes, and (best-effort) video
# recordings from the Essex, CT Agenda Center for meetings within the
# past N days (and up to M days ahead).
#
# USAGE:
#   python3 scripts/download-essex-agendas.py [options]
#
# REQUIREMENTS:
...
#     1. Fetches the Essex CT Agenda Center search endpoint with a
...
#     3. Downloads PDFs to beat-archive/essex-agendas/YYYY-MM/
#
...
# Chester elsewhere in this repo):
#   Hub:     https://www.essexct.gov/AgendaCenter
#   Search:  https://www.essexct.gov/AgendaCenter/Search/?term=&CIDs=all
#              &startDate=MM/DD/YYYY&endDate=MM/DD/YYYY&dateRange=Custom&dateSelector=0
#   Agenda:  https://www.essexct.gov/AgendaCenter/ViewFile/Agenda/_MMDDYYYY-ID
#   Minutes: https://www.essexct.gov/AgendaCenter/ViewFile/Minutes/_MMDDYYYY-ID
#
# VIDEO NOTE — No reliable per-meeting video source was found for Essex.
#   Checked directly before writing this script:
#     - No YouTube/Vimeo/Facebook-Live/PEG link anywhere on essexct.gov.
#     - The regional Cablecast (Tightrope Media Systems) instance at
#       reflect-vsctv.cablecast.tv — the same platform already used for
#       Durham, Middlefield, and Chester elsewhere in this repo — does
#       carry an "Essex" project (ID 9), but it's entirely Essex
#       Congregational Church service recordings, not municipal meetings.
#       Direct searches for "Essex Board of Selectmen", "Essex Board of
#       Finance", "Essex Planning", and "Essex Zoning" all return zero
#       results.
#     - A YouTube search for "Essex CT Board of Selectmen meeting" returns
#       nothing from this Essex (results are for Essex Junction, VT and
#       neighboring CT towns).
#   This is the same gap already found for Durham, Middlefield, and
#   Chester — video search is included anyway for completeness and left
#   OFF by default (--include-video/--video-only to try it) — expect it
#   to find nothing until/unless Essex starts recording meetings
...
# --- Configuration ---
BASE_URL = "https://www.essexct.gov"
SEARCH_URL = f"{BASE_URL}/AgendaCenter/Search/"
CABLECAST_BASE = "https://reflect-vsctv.cablecast.tv/cablecastapi/v1"
OUTPUT_DIR = "beat-archive/essex-agendas"
DAYS_BACK = 4
DAYS_AHEAD = 7
DELAY_SECONDS = 0.8

UA = "Essex-CT-Agendas-Downloader/1.0 (journalism research)"
...
def collect_videos(cutoff, future_limit, board_filter, known_boards):
    ...
    for board in sorted(boards):
        for show in fetch_cablecast_shows(search=f"Essex {board}"):
            if show["id"] in seen_show_ids:
                continue
            title = show.get("title") or ""
            if "essex" not in title.lower() or board.lower() not in title.lower():
                continue
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Download Essex CT municipal agendas, minutes, and (best-effort) "
...
#    python3 scripts/download-essex-agendas.py --dry-run
#
# 2. Just Board of Selectmen:
#    python3 scripts/download-essex-agendas.py --board "Board of Selectman"
#
# 3. Change the lookback window:
#    python3 scripts/download-essex-agendas.py --days 14
#
# 4. Also try Cablecast video search (see VIDEO NOTE — expect little/nothing):
#    python3 scripts/download-essex-agendas.py --include-video
#
# 5. Run on a schedule (cron — evening):
#    0 20 * * 1-5 cd /path/to/repo && python3 scripts/download-essex-agendas.py
