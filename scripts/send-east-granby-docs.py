#!/usr/bin/env python3
# send-east-granby-docs.py
# Run the East Granby CT agenda downloader and email all new files.
#
# CONFIGURATION:
...
SCRIPT     = os.path.join(REPO_DIR, "scripts", "download-east-granby-agendas.py")
OUTPUT_DIR = os.path.join(REPO_DIR, "beat-archive", "east-granby-agendas")
...
def run_downloader():
    print("Running download-east-granby-agendas.py ...")
...
def send_email(files, downloader_output):
    ...
    subject = (
        f"East Granby CT meeting docs — {datetime.date.today().strftime('%B %-d, %Y')} "
    ...
    body = (
        f"East Granby CT agenda/minutes download — {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}\n\n"
