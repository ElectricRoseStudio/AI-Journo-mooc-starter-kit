#!/usr/bin/env python3
# send-berlin-docs.py
# Run the Berlin CT agenda downloader and email all new files.
#
# CONFIGURATION:
...
SCRIPT     = os.path.join(REPO_DIR, "scripts", "download-berlin-agendas.py")
OUTPUT_DIR = os.path.join(REPO_DIR, "beat-archive", "berlin-agendas")
...
def run_downloader():
    print("Running download-berlin-agendas.py ...")
...
def send_email(files, downloader_output):
    subject = (
        f"Berlin CT meeting docs — {datetime.date.today().strftime('%B %-d, %Y')} "
    ...
    body = (
        f"Berlin CT agenda/minutes download — {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}\n\n"
