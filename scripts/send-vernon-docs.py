#!/usr/bin/env python3
# send-vernon-docs.py
# Run the Vernon CT agenda downloader and email all new files.
#
# CONFIGURATION:
...
SCRIPT     = os.path.join(REPO_DIR, "scripts", "download-vernon-agendas.py")
OUTPUT_DIR = os.path.join(REPO_DIR, "beat-archive", "vernon-agendas")
...
def run_downloader():
    print("Running download-vernon-agendas.py ...")
...
def send_email(files, downloader_output):
    ...
    subject = (
        f"Vernon CT meeting docs — {datetime.date.today().strftime('%B %-d, %Y')} "
    ...
    body = (
        f"Vernon CT agenda/minutes download — {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}\n\n"
