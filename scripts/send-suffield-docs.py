#!/usr/bin/env python3
# send-suffield-docs.py
# Run the Suffield CT agenda downloader and email all new files.
#
# CONFIGURATION:
...
SCRIPT     = os.path.join(REPO_DIR, "scripts", "download-suffield-agendas.py")
OUTPUT_DIR = os.path.join(REPO_DIR, "beat-archive", "suffield-agendas")
CITY_NAME  = "SUFFIELD"
...
def run_downloader():
    print("Running download-suffield-agendas.py ...")
...
def send_email(files, downloader_output, biz_table=""):
    ...
    subject = (
        f"Suffield CT meeting docs — {datetime.date.today().strftime('%B %-d, %Y')} "
    ...
    body = (
        f"Suffield CT agenda/minutes download — {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}\n\n"
