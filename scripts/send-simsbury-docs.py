#!/usr/bin/env python3
# send-simsbury-docs.py
# Run the Simsbury CT agenda downloader and email all new files.
#
# CONFIGURATION:
...
SCRIPT     = os.path.join(REPO_DIR, "scripts", "download-simsbury-agendas.py")
OUTPUT_DIR = os.path.join(REPO_DIR, "beat-archive", "simsbury-agendas")
CITY_NAME  = "SIMSBURY"
...
def run_downloader():
    print("Running download-simsbury-agendas.py ...")
...
def send_email(files, downloader_output, biz_table=""):
    subject = (
        f"Simsbury CT meeting docs — {datetime.date.today().strftime('%B %-d, %Y')} "
    ...
    body = (
        f"Simsbury CT agenda/minutes download — {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}\n\n"
