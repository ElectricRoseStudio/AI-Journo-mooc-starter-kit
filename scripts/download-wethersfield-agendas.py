#!/usr/bin/env python3
# download-wethersfield-agendas.py
# Download municipal meeting agendas, minutes, and video recordings from
# the Wethersfield, CT Agenda Center for meetings within the past N days
# (and up to M days ahead).
#
# USAGE:
#   python3 scripts/download-wethersfield-agendas.py [options]
#
# REQUIREMENTS:
...
#     1. Fetches the Wethersfield CT Agenda Center search endpoint with a
...
#     3. Downloads PDFs to beat-archive/wethersfield-agendas/YYYY-MM/
#
#   Video (--include-video or --video-only; off by default):
#     4. Downloads recent videos from the "Wethersfield Government Access
#        TV" YouTube channel using yt-dlp — this channel posts real,
#        current per-meeting recordings (titled e.g. "Planning and Zoning
#        Commission  April 21, 2026"), confirmed directly before writing
#        this script.
#
...
# Chester/Essex/Deep River/Newington/Norwich elsewhere in this repo):
#   Hub:     https://www.wethersfieldct.gov/AgendaCenter
#   Search:  https://www.wethersfieldct.gov/AgendaCenter/Search/?term=&CIDs=all
#              &startDate=MM/DD/YYYY&endDate=MM/DD/YYYY&dateRange=Custom&dateSelector=0
#   Agenda:  https://www.wethersfieldct.gov/AgendaCenter/ViewFile/Agenda/_MMDDYYYY-ID
#   Minutes: https://www.wethersfieldct.gov/AgendaCenter/ViewFile/Minutes/_MMDDYYYY-ID
#   YouTube: https://www.youtube.com/channel/UCfuf-C0gYVVxmD5iIfFTnsQ
#              ("Wethersfield Government Access TV" — not linked from the
#              town's own site; found via search, channel URL confirmed
#              directly against its /videos listing)
...
# --- Configuration ---
BASE_URL = "https://www.wethersfieldct.gov"
SEARCH_URL = f"{BASE_URL}/AgendaCenter/Search/"
YOUTUBE_CHANNEL = "https://www.youtube.com/channel/UCfuf-C0gYVVxmD5iIfFTnsQ"
OUTPUT_DIR = "beat-archive/wethersfield-agendas"
DAYS_BACK = 4
DAYS_AHEAD = 7
DELAY_SECONDS = 0.8
...
UA = "Wethersfield-CT-Agendas-Downloader/1.0 (journalism research)"
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Download Wethersfield CT municipal agendas, minutes, and video "
...
#    python3 scripts/download-wethersfield-agendas.py --dry-run
#
# 2. Just Town Council:
#    python3 scripts/download-wethersfield-agendas.py --board "Town Council"
#
# 3. PDFs only (no video — the default; --include-video/--video-only turn it on):
#    python3 scripts/download-wethersfield-agendas.py
#
# 4. Video only:
#    python3 scripts/download-wethersfield-agendas.py --video-only
#
# 5. Change the lookback window:
#    python3 scripts/download-wethersfield-agendas.py --days 14
#
# 6. Run on a schedule (cron — evening):
#    0 19 * * 1-5 cd /path/to/repo && python3 scripts/download-wethersfield-agendas.py --include-video
