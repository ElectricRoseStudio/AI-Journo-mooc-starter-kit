#!/usr/bin/env python3
# send-east-hartford-docs.py
# Run the East Hartford CT agenda downloader and email all new files.
#
# CONFIGURATION:
...
TO_ADDRESS   = "michael.lemanski@patch.com"

REPO_DIR   = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
LOG_PATH   = os.path.join(REPO_DIR, "beat-archive", "send-log.txt")
SCRIPT     = os.path.join(REPO_DIR, "scripts", "download-east-hartford-agendas.py")
OUTPUT_DIR = os.path.join(REPO_DIR, "beat-archive", "east-hartford-agendas")

ATTACH_EXTENSIONS = {".pdf", ".csv"}
...
def run_downloader():
    print("Running download-east-hartford-agendas.py ...")
    result = subprocess.run(
        [sys.executable, SCRIPT, "--recordings"],
        capture_output=True,
        text=True,
        cwd=REPO_DIR,
    )
...
def send_email(files, downloader_output):
    subject = (
        f"East Hartford CT meeting docs — {datetime.date.today().strftime('%B %-d, %Y')} "
    ...
    body = (
        f"East Hartford CT agenda/minutes download — {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}\n\n"
