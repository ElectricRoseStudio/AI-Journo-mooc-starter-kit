#!/usr/bin/env python3
# send-fairfield-docs.py
# Run the Fairfield CT agenda downloader and email all new files.
#
# CONFIGURATION:
...
TO_ADDRESS   = "alfred.branch@patch.com"

REPO_DIR   = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
LOG_PATH   = os.path.join(REPO_DIR, "beat-archive", "send-log.txt")
SCRIPT     = os.path.join(REPO_DIR, "scripts", "download-fairfield-agendas.py")
OUTPUT_DIR = os.path.join(REPO_DIR, "beat-archive", "fairfield-agendas")
CITY_NAME  = "FAIRFIELD"
...
def run_downloader():
    print("Running download-fairfield-agendas.py ...")
...
def send_email(files, downloader_output, biz_table=""):
    subject = (
        f"Fairfield CT meeting docs — {datetime.date.today().strftime('%B %-d, %Y')} "
    ...
    body = (
        f"Fairfield CT agenda/minutes download — {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}\n\n"
