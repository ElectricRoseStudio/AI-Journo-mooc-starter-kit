#!/usr/bin/env python3
# download-norwich-agendas.py
# Download municipal meeting agendas, minutes, and video recordings from
# the Norwich, CT Agenda Center for meetings within the past N days
# (and up to M days ahead).
#
# USAGE:
#   python3 scripts/download-norwich-agendas.py [options]
#
# REQUIREMENTS:
...
#     1. Fetches the Norwich CT Agenda Center search endpoint with a
...
#     3. Downloads PDFs to beat-archive/norwich-agendas/YYYY-MM/
#
#   Video (--include-video or --video-only; off by default):
#     4. Downloads recent videos from the City of Norwich YouTube channel
#        using yt-dlp — this channel posts real, current per-meeting
#        recordings (titled e.g. "2026-08-03 Council Meeting"), confirmed
...
# Chester/Essex/Deep River/Newington elsewhere in this repo):
#   Hub:     https://www.norwichct.gov/AgendaCenter
#   Search:  https://www.norwichct.gov/AgendaCenter/Search/?term=&CIDs=all
#              &startDate=MM/DD/YYYY&endDate=MM/DD/YYYY&dateRange=Custom&dateSelector=0
#   Agenda:  https://www.norwichct.gov/AgendaCenter/ViewFile/Agenda/_MMDDYYYY-ID
#   Minutes: https://www.norwichct.gov/AgendaCenter/ViewFile/Minutes/_MMDDYYYY-ID
#   YouTube: https://www.youtube.com/@citynorwichct
#
# NOTE: The town's own /AgendaCenter link on norwichct.org 301-redirects
#   every request to the same path on norwichct.gov, so this script uses
#   the .gov domain directly to skip that extra hop.
...
# --- Configuration ---
BASE_URL = "https://www.norwichct.gov"
SEARCH_URL = f"{BASE_URL}/AgendaCenter/Search/"
YOUTUBE_CHANNEL = "https://www.youtube.com/@citynorwichct"
OUTPUT_DIR = "beat-archive/norwich-agendas"
DAYS_BACK = 4
DAYS_AHEAD = 7
DELAY_SECONDS = 0.8
...
UA = "Norwich-CT-Agendas-Downloader/1.0 (journalism research)"
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Download Norwich CT municipal agendas, minutes, and video "
...
#    python3 scripts/download-norwich-agendas.py --dry-run
#
# 2. Just City Council:
#    python3 scripts/download-norwich-agendas.py --board "City Council"
#
# 3. PDFs only (no video — the default; --include-video/--video-only turn it on):
#    python3 scripts/download-norwich-agendas.py
#
# 4. Video only:
#    python3 scripts/download-norwich-agendas.py --video-only
#
# 5. Change the lookback window:
#    python3 scripts/download-norwich-agendas.py --days 14
#
# 6. Run on a schedule (cron — evening):
#    0 19 * * 1-5 cd /path/to/repo && python3 scripts/download-norwich-agendas.py --include-video
