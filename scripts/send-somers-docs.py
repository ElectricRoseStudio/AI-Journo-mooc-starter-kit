#!/usr/bin/env python3
# send-somers-docs.py
# Run the Somers CT agenda downloader and email all new files.
#
# CONFIGURATION:
...
SCRIPT     = os.path.join(REPO_DIR, "scripts", "download-somers-agendas.py")
OUTPUT_DIR = os.path.join(REPO_DIR, "beat-archive", "somers-agendas")
...
def run_downloader():
    print("Running download-somers-agendas.py ...")
...
def send_email(files, downloader_output):
    ...
    subject = (
        f"Somers CT meeting docs — {datetime.date.today().strftime('%B %-d, %Y')} "
    ...
    body = (
        f"Somers CT agenda/minutes download — {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}\n\n"
