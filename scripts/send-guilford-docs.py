#!/usr/bin/env python3
# send-guilford-docs.py
# Run the Guilford CT agenda downloader and email all new files.
#
# CONFIGURATION:
...
SCRIPT     = os.path.join(REPO_DIR, "scripts", "download-guilford-agendas.py")
OUTPUT_DIR = os.path.join(REPO_DIR, "beat-archive", "guilford-agendas")
CITY_NAME  = "GUILFORD"
...
def run_downloader():
    print("Running download-guilford-agendas.py --include-video ...")
...
def send_email(files, downloader_output, biz_table=""):
    ...
    subject = (
        f"Guilford CT meeting docs — {datetime.date.today().strftime('%B %-d, %Y')} "
    ...
    body = (
        f"Guilford CT agenda/minutes download — {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}\n\n"
