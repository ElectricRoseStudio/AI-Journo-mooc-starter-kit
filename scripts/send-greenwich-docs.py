#!/usr/bin/env python3
# send-greenwich-docs.py
# Run the Greenwich CT agenda downloader and email all new files.
#
# CONFIGURATION:
...
TO_ADDRESS   = "richard.kaufman@patch.com"

REPO_DIR   = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
LOG_PATH   = os.path.join(REPO_DIR, "beat-archive", "send-log.txt")
SCRIPT     = os.path.join(REPO_DIR, "scripts", "download-greenwich-agendas.py")
OUTPUT_DIR = os.path.join(REPO_DIR, "beat-archive", "greenwich-agendas")
...
def run_downloader():
    print("Running download-greenwich-agendas.py --include-video ...")
...
def send_email(files, downloader_output):
    ...
    subject = (
        f"Greenwich CT meeting docs — {datetime.date.today().strftime('%B %-d, %Y')} "
    ...
    body = (
        f"Greenwich CT agenda/minutes download — {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}\n\n"
