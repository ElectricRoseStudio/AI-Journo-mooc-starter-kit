#!/usr/bin/env python3
# send-norwalk-docs.py
# Run the Norwalk CT agenda downloader and email all new files.
#
# CONFIGURATION:
...
SCRIPT     = os.path.join(REPO_DIR, "scripts", "download-norwalk-agendas.py")
...
def run_downloader():
    print("Running download-norwalk-agendas.py ...")
...
def send_email(files, downloader_output, biz_table=""):
    ...
    subject = (
        f"Norwalk CT meeting docs — {datetime.date.today().strftime('%B %-d, %Y')} "
    ...
    body = (
        f"Norwalk CT agenda/minutes download — {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}\n\n"
