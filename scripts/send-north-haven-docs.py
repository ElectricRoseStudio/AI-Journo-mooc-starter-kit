#!/usr/bin/env python3
# send-north-haven-docs.py
# Run the North Haven CT agenda downloader and email all new files.
#
# CONFIGURATION:
...
SCRIPT     = os.path.join(REPO_DIR, "scripts", "download-north-haven-agendas.py")
OUTPUT_DIR = os.path.join(REPO_DIR, "beat-archive", "north-haven-agendas")
...
def run_downloader():
    print("Running download-north-haven-agendas.py ...")
...
def send_email(files, downloader_output):
    ...
    subject = (
        f"North Haven CT meeting docs — {datetime.date.today().strftime('%B %-d, %Y')} "
    ...
    body = (
        f"North Haven CT agenda/minutes download — {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}\n\n"
