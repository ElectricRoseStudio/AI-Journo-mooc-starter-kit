#!/usr/bin/env python3
# send-cheshire-docs.py
# Run the Cheshire CT agenda downloader and email all new files.
#
# CONFIGURATION:
...
SCRIPT     = os.path.join(REPO_DIR, "scripts", "download-cheshire-agendas.py")
OUTPUT_DIR = os.path.join(REPO_DIR, "beat-archive", "cheshire-agendas")
...
def run_downloader():
    print("Running download-cheshire-agendas.py ...")
...
def send_email(files, downloader_output):
    ...
    subject = (
        f"Cheshire CT meeting docs — {datetime.date.today().strftime('%B %-d, %Y')} "
    ...
    body = (
        f"Cheshire CT agenda/minutes download — {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}\n\n"
