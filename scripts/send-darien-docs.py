#!/usr/bin/env python3
# send-darien-docs.py
# Run the Darien CT agenda downloader and email all new files.
#
# CONFIGURATION:
...
SCRIPT     = os.path.join(REPO_DIR, "scripts", "download-darien-agendas.py")
OUTPUT_DIR = os.path.join(REPO_DIR, "beat-archive", "darien-agendas")
...
def run_downloader():
    print("Running download-darien-agendas.py --include-video ...")
...
def send_email(files, downloader_output):
    ...
    subject = (
        f"Darien CT meeting docs — {datetime.date.today().strftime('%B %-d, %Y')} "
    ...
    body = (
        f"Darien CT agenda/minutes download — {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}\n\n"
