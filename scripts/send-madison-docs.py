#!/usr/bin/env python3
# send-madison-docs.py
# Run the Madison CT agenda downloader and email all new files.
#
# CONFIGURATION:
...
SCRIPT     = os.path.join(REPO_DIR, "scripts", "download-madison-agendas.py")
OUTPUT_DIR = os.path.join(REPO_DIR, "beat-archive", "madison-agendas")
...
def run_downloader():
    print("Running download-madison-agendas.py ...")
...
def send_email(files, downloader_output):
    subject = (
        f"Madison CT meeting docs — {datetime.date.today().strftime('%B %-d, %Y')} "
    ...
    body = (
        f"Madison CT agenda/minutes download — {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}\n\n"
