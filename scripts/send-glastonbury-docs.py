#!/usr/bin/env python3
# send-glastonbury-docs.py
# Run the Glastonbury CT agenda downloader and email all new files.
#
# CONFIGURATION:
...
TO_ADDRESS   = "chris.dehnel@patch.com"

REPO_DIR   = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
LOG_PATH   = os.path.join(REPO_DIR, "beat-archive", "send-log.txt")
SCRIPT     = os.path.join(REPO_DIR, "scripts", "download-glastonbury-agendas.py")
OUTPUT_DIR = os.path.join(REPO_DIR, "beat-archive", "glastonbury-agendas")
...
def run_downloader():
    print("Running download-glastonbury-agendas.py ...")
...
def send_email(files, downloader_output, biz_table=""):
    ...
    subject = (
        f"Glastonbury CT meeting docs — {datetime.date.today().strftime('%B %-d, %Y')} "
    ...
    body = (
        f"Glastonbury CT agenda/minutes download — {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}\n\n"
