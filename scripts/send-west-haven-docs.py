#!/usr/bin/env python3
# send-west-haven-docs.py
# Run the West Haven CT agenda downloader and email all new files.
#
# CONFIGURATION:
...
SCRIPT     = os.path.join(REPO_DIR, "scripts", "download-west-haven-agendas.py")
OUTPUT_DIR = os.path.join(REPO_DIR, "beat-archive", "west-haven-agendas")
...
def run_downloader():
    print("Running download-west-haven-agendas.py ...")
...
def send_email(files, downloader_output, biz_table=""):
    ...
    subject = (
        f"West Haven CT meeting docs — {datetime.date.today().strftime('%B %-d, %Y')} "
    ...
    body = (
        f"West Haven CT agenda/minutes download — {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}\n\n"
